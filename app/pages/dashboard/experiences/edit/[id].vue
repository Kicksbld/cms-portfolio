  ...
  <NuxtLayout name="dashboard">
    <template #header>
      <DashboardPageHeader
        ...
        description="Update your professional or educational experience"
      />
    </template>
    ...
    <Loader
      ...
      description="Please wait while we fetch the experience details."
    />

    <div v-else-if="experience" class="space-y-6 max-w-3xl">
      <!-- Experience Details Card -->
      <Card>
        <CardHeader>
          <CardTitle>Experience Details</CardTitle>
          <CardDescription>
            Update the details for your experience
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <div class="flex items-center gap-3">
        <Button
          type="button"
          :disabled="experiencesStore.loading || !formData.title"
          @click="handleSubmit"
          class="gap-2"
        >
          <Spinner v-if="experiencesStore.loading" />
          <Save v-else class="h-4 w-4" />
          {{ experiencesStore.loading ? "Updating..." : "Update Experience" }}
        </Button>
        <Button
          type="button"
          variant="outline"
    ...
    <div v-else class="text-center py-12">
      <p class="text-muted-foreground">Experience not found</p>
    </div>
  </NuxtLayout>
</template>
...
import type { ExperienceType } from "~/types/experiences";
...
const formData = reactive({
  title: "",
  type: "professionnel" as ExperienceType,
  location: "",
  start_date: "",
  end_date: "",
  description: "",
...
onMounted(async () => {
  ...
  try {
    experience.value = await experiencesStore.fetchExperience(experienceId.value);

    // Populate form
    formData.title = experience.value.title;
    formData.type = experience.value.type;
    formData.location = experience.value.location || "";
    formData.start_date = experience.value.start_date || "";
    formData.end_date = experience.value.end_date || "";
    formData.description = experience.value.description || "";

    // Set current position checkbox if end_date is null
    if (!experience.value.end_date) {
...
const handleSubmit = async () => {
  ...
  try {
    await experiencesStore.updateExperience(experienceId.value, {
